...
def about(topic):
    ...
    def select(paragraphs):
        split_words = remove_punctuation(paragraphs)
        split_words = lower(split_words)
        split_words = split(split_words)
        for word in split_words:
            if word in topic:
                return True
            elif word == split_words[len(split_words)-1]:
                return False
    return select
    # END PROBLEM 2
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    min_diff = min(valid_words, key=lambda item: diff_function(typed_word, item, limit))
    diff_of_words = []
    for word in valid_words:
        diff = diff_function(typed_word, word, limit)
        diff_of_words.append(diff)
    if typed_word in valid_words:
        return typed_word
    elif min(diff_of_words) > limit:
        return typed_word
    elif min(diff_of_words) <= limit:
        return min_diff
    # END PROBLEM 5
...
def sphinx_switches(start, goal, limit):
    ...
    if start == goal:
        ...
        return 1 + sphinx_switches(start[1:], goal[1:], limit-1) #when u need to count something, make sure to have 1+
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)):
        times = []
        for a, b in zip(times_per_player[i], times_per_player[i][1:]):
            times.append(b-a)
        lst.append(times)
...
def fastest_words(game):
    ...
    for i in player_indices:
        ...
        for word in word_indices:
            fast_player = min(player_indices, key=lambda i: time(game, i, word))
            if i == fast_player:
                fastest.append(word_at(game, word))
        lst_of_lsts.append(fastest)
    return lst_of_lsts
    # END PROBLEM 10
