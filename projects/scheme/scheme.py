...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        lambda_func = lambda expr: scheme_eval(expr, env)
        evaluated_pair = expr.map(lambda_func)
        validate_procedure(evaluated_pair.first)
        return scheme_apply(evaluated_pair.first, evaluated_pair.rest, env)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        # BEGIN PROBLEM 3
        curr_args = args
        while curr_args is not nil:
            arguments_list.append(curr_args.first)
