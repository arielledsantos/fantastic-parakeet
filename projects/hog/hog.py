...
def piggy_points(score):
    ...
    "*** YOUR CODE HERE ***"
    score_sq = score**2
    min_digit = score_sq % 10
    while score_sq:
        digit = score_sq % 10
        min_digit = min(min_digit, digit)
        score_sq = score_sq //10
    return min_digit + 3
...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    if len(str(player_score)) > 2:
        p_two_digits = player_score // 10
        p_digits = p_two_digits //10, p_two_digits % 10
    else:
        p_digits = player_score // 10, player_score % 10
    
    if len(str(opponent_score)) > 2:
        o_two_digits = opponent_score // 10
        o_digits = o_two_digits //10, o_two_digits % 10
    else:
        o_digits = opponent_score // 10, opponent_score % 10
      
    return p_digits[0] < o_digits[0] and p_digits[1] < o_digits[1]
    # END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while n >= 1:
        ...
        roll_avg = averaged_dice(n, dice)
        if highest_avg < roll_avg:
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if piggypoints_strategy(score, opponent_score, cutoff, num_rolls) == 0 and more_boar(score, opponent_score) == False:
        return 0
